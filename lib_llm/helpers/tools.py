...
def search_restaurants(args):
    ...
    # if not mood and not preference and not cuisine:
    #     return {
    #         "is_llm_needed": True,
    #         "data": "I need to know more about what you're in the mood for. Would you prefer something spicy, comforting, light, or something else?"
    #     }
    ...
    if not api_response : 
        return {
            "is_llm_needed": True,
            "data": "No resturants found for your search"
        }
    ...
    # Format the results with personalized recommendations
    result = "Based on what you're in the mood for, I've found these great options:\n\n"
    for restaurant in matching_restaurants[:5]:
        result += f"• {restaurant['name']} \n"

    result += "Would you like to know more about any of these restaurants' menus?"
...
def search_cuisines(args):
    ...
    if not api_response : 
        return {
            "is_llm_needed": True,
            "data": "No cuisnes found for your search"
        }


    matching_cusines = api_response.get("cuisines", [])

    # Format the results with personalized recommendations
    result = "Based on what your preferred cuisines, I've found these great options:\n\n"
    for cuisine in matching_cusines:
        result += f"• {cuisine} \n"
...
def get_dietary_preferences(args):
    ...
    if not api_response : 
        return {
            "is_llm_needed": True,
            "data": "No dietry preferences found for your search"
        }
    ...
    # Format the results with personalized recommendations
    result = "These are the available dietry pereferences :\n\n"
    for dietry_preference in matching_dietary:
        result += f"• {dietry_preference} \n"
...
def get_restaurant_menu(args):
    ...
    if not api_response : 
        return {
            "is_llm_needed": True,
            "data": "No resturant menue found for your search"
        }
    ...
    print(f"Restaurant Found: {restaurant}. Resturant Name: {restaurant_name}")
    if not restaurant:
        return {
            "is_llm_needed": True,
